package com.shunsong.controller.system.user;
...
import com.shunsong.util.GetPinyin;
import com.shunsong.util.Jurisdiction;
import com.shunsong.util.ObjectExcelRead;
...
public class UserController extends BaseController {
	...
	public ModelAndView listUsersView(Page page)throws Exception{
		ModelAndView mv = this.getModelAndView();
		PageData pd = new PageData();
		pd = this.getPageData();
		String keywords = pd.getString("keywords");				//关键词检索条件
		...
		page.setPd(pd);
		List<PageData>	userList = userService.listUsers(page);	//列出用户列表
		mv.setViewName("system/user/user_list_view");
		mv.addObject("userList", userList);
		mv.addObject("pd", pd);
		return mv;
	}
	 ...
	 * @throws Exception
	 */
	@RequestMapping(value="/goChangeCurrentDept")
	public ModelAndView goChangeCurrentDept() throws Exception{
		ModelAndView mv = this.getModelAndView();
		PageData pd = new PageData();
		pd = this.getPageData();
		String fx = pd.getString("fx");//顶部修改个人资料
		...
		List<PageData> deptList = this.userService.listUserDepts(pd);
		
		mv.setViewName("system/user/user_change_current_dept");
		mv.addObject("msg", "changeUserDept");
		mv.addObject("deptList", deptList);
		return mv;
	}
	
	@RequestMapping(value="/changeUserDept")
	@ResponseBody
	public Object changeUserDept() throws Exception{
		...
		String strDeptID = pd.getString("DEPARTMENT_ID");
		String nowDeptID = Jurisdiction.getDeptID();
		if(!nowDeptID.equals(strDeptID)){
			//切换部门 DEPARTMENT_ID
			PageData pdDept = this.departmentService.findById(pd);
			String strName = pdDept.getString("NAME");
			
			if(null != Jurisdiction.getSession().getAttribute(Const.SESSION_USERNAME)){
				Session session = Jurisdiction.getSession();	//以下清除session缓存
				
				User user = (User)session.getAttribute(Const.SESSION_USER);
				user.setDEPARTMENT_ID(strDeptID);
				user.setDEPARTMENT_NAME(strName);
				
				session.setAttribute(Const.SESSION_USER, user);
				session.setAttribute(Const.SESSION_DEPARTMENTID, strDeptID);				//放入部门ID到session
				session.setAttribute(Const.SESSION_DEPARTMENTNAME, strName);				//放入部门名到session
				
				User userrol = (User)session.getAttribute(Const.SESSION_USERROL);
				userrol.setDEPARTMENT_ID(strDeptID);
				userrol.setDEPARTMENT_NAME(strName);
				session.setAttribute(Const.SESSION_USERROL, userrol);
				
				pd.put("DEPARTMENT_NAME", strName);
			}
		}
	 ...
	 * @throws Exception
	 */
	@RequestMapping(value="/goListUsersChoose")
	public ModelAndView goListUsersChoose(Page page)throws Exception{
		ModelAndView mv = this.getModelAndView();
		PageData pd = new PageData();
		pd = this.getPageData();
//		page.setPd(pd);
//		List<PageData>	userList = userService.listUsers(page);	//列出用户列表
		mv.setViewName("system/user/user_list_choose_easyui");
//		mv.addObject("userList", userList);
		mv.addObject("pd", pd);
		return mv;
	}
	...
	/**列表
	 * @param page
	 * @throws Exception_zhongrui MOULD_NO
	 */
	@RequestMapping(value="/listUserData")
	public String listUserData(Page page,HttpServletResponse response) throws Exception{
		logBefore(logger, Jurisdiction.getUsername()+"列表user");
		PageData pd = new PageData();
		pd = this.getPageData();
		pd.put("NOW_USER", Jurisdiction.getUserId());
		...
		page.setCurrentResult(intPage);		
		page.setShowCount(pd.getInt("pageSize"));
		page.setPd(pd);
		List<PageData>	userList = userService.listUsers(page);	//列出用户列表
	...
	/**列表
	 * @param page 下拉框用list_user_data
	 * @throws Exception_zhongrui 
	 */
	@RequestMapping(value="/list_user_data")
	public String list_user_data(Page page,HttpServletResponse response) throws Exception{
		logBefore(logger, Jurisdiction.getUsername()+"列表Demand");
//		ModelAndView mv = this.getModelAndView();
		PageData pd = new PageData();
		pd = this.getPageData();
		if(!"admin".equals(Jurisdiction.getUsername())){
			if(null != pd.get("CURRENT_USER_FLAG")){
				pd.put("CURRENT_USER", Jurisdiction.getUserId());
			}
		}
		if("Y".equals(pd.getString("matchUserFlag"))){
			pd.put("MatchUser", Jurisdiction.getUserId());
		}
		page.setPd(pd);
		page.setShowCount(1000);
		List<PageData>	userList = userService.listUsers(page);	//列出用户列表
		if("Y".equals(pd.getString("emptyFlag"))){
			PageData empty = new PageData();
			empty.put("USER_ID", null);
			empty.put("NAME", "全部");
			userList.add(0, empty);;
		}
	...
	@RequestMapping( value = "/getStaffPictureUrl", method = RequestMethod.POST)
	@ResponseBody
	public Object getStaffPictureUrl(HttpServletResponse response) throws Exception{
		Map<String,String> map = new HashMap<String,String>();
		...
		PageData pdAttachment = userService.getUserStaffUrl(pd);
	...
	public ModelAndView userListSelect(Page page)throws Exception{
		ModelAndView mv = this.getModelAndView();
		PageData pd = new PageData();
		pd = this.getPageData();
		page.setPd(pd);
		List<PageData>	userList = null;
		if("CY".equals(pd.getString("flag"))){
			userList = userService.listUserByProcessId(page);	//列出用户列表
		}else{
			userList = userService.listUsers(page);
		}
		mv.setViewName("system/user/user_list_select");

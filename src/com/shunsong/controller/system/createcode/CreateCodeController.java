package com.shunsong.controller.system.createcode;
...
import com.shunsong.service.system.createcode.CreateCodeManager;
import com.shunsong.util.AppUtil;
import com.shunsong.util.DateUtil;
import com.shunsong.util.DelAllFile;
import com.shunsong.util.FileDownload;
import com.shunsong.util.FileZip;
import com.shunsong.util.Freemarker;
import com.shunsong.util.Jurisdiction;
import com.shunsong.util.PageData;
import com.shunsong.util.PathUtil;
import com.shunsong.util.database.ColomnMetadata;
import com.shunsong.util.database.ConfigDB;
import com.shunsong.util.database.DatabaseMetaQuery;
import com.shunsong.util.database.TableMetadata;
...
public class CreateCodeController extends BaseController {
	...
	public void proCode(HttpServletResponse response) throws Exception{
		...
		List<TableMetadata> tables = DatabaseMetaQuery.getTableMetas(tabletop+objectName.toLowerCase());
		
		List<ColomnMetadata> cols = tables.get(0).getCols();
		
		List<String[]> fieldList = new ArrayList<String[]>();   	//属性集合			========参数4
		
		for(ColomnMetadata col : cols){
			String[] colsStr = new String[6];
			colsStr[0] = col.getColName();
			colsStr[1] = col.getColType();
			colsStr[2] = col.getColDesc();
			colsStr[3] = "是";
			colsStr[4] = "";
			colsStr[5] = "";
						
			
			fieldList.add(colsStr);	//属性放到集合里面
		}

package com.shunsong.controller.system.head;
...
import com.shunsong.service.system.fhsms.FhsmsManager;
...
import com.shunsong.util.SmsUtil;
import com.shunsong.util.Tools;
import com.shunsong.util.Watermark;
import com.shunsong.util.mail.SimpleMailSender;
...
public class HeadController extends BaseController {
	...
	public Object getList() {
		...
		try {
			...
			pds.put("DEPTNAME", Jurisdiction.getDeptName());				
			...
			PageData userWorkTimePd = new PageData();
			userWorkTimePd.put("TYPE", "USER_WORK_TIME");
			userWorkTimePd.put("USER_ID", Jurisdiction.getUserId());
			List<PageData> listUserWorkTodo = null;
			if(CollectionUtils.isEmpty(listUserWorkTodo)){
				map.put("userWorktimeCount",0);
			}else{

				map.put("userWorktimeCount",listUserWorkTodo.get(0).get("mess"));//站内信未读总数
			}
			...
			if(null != strWEBSOCKET && !"".equals(strWEBSOCKET)){
				...
				if(strIW.length == 7){
					...
					map.put("plantipsadress", strIW[5]+":"+strIW[6]);//生产计划排程设置服务器IP和端口
				}
			}
		} catch (Exception e) {
	...
	@RequestMapping(value="/getMessList")
	@ResponseBody
	public Object getMessList() {
		PageData pd = new PageData();
		Map<String,Object> map = new HashMap<String,Object>();
		try {
			pd = this.getPageData();
	 ...
	 * @return
	 */
	@RequestMapping(value="/getUserWorktimeCount")
	@ResponseBody
	public Object getUserWorktimeCount() {
		PageData pd = new PageData();
		Map<String,Object> map = new HashMap<String,Object>();
		try {
			PageData userWorkTimePd = new PageData();
	 ...
	 * @return
	 */
	@RequestMapping(value="/getEstimateCount")
	@ResponseBody
	public Object getEstimateCount() {
		PageData pd = new PageData();
		Map<String,Object> map = new HashMap<String,Object>();
		try {
			pd.put("USER_ID", Jurisdiction.getUserId());
	...
	public ModelAndView saveSys() throws Exception{
		...
		Tools.writeFile(Const.COMNAME,pd.getString("COMNAME"));	//写入系统名称

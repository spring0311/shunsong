...
public class Page {
	...
	private String pageAjaxStr;		//最终页面显示的底部翻页导航，详细见：getPageStr();
	...
	//拼接分页 页面及JS函数
	public String getPageAjaxStr(String strFuncName) {
		StringBuffer sb = new StringBuffer();
		if(totalResult>0){
			sb.append("	<ul class=\"pagination pull-right no-margin\">\n");
			if(currentPage==1){
			...
			sb.append(" var showPageCount=10;\n");
			sb.append(" var currentPageNum;\n");	
			//换页函数
			sb.append("function nextPage(page){\n");
			sb.append(" top.jzts();\n");			
			sb.append(strFuncName+"(page,showPageCount);\n");			
			sb.append("}\n");
			
			//调整每页显示条数
			sb.append("function changeCount(value){\n");
			sb.append(" top.jzts();\n");
			sb.append(" showPageCount = value;\n");
			sb.append(strFuncName+"(currentPageNum,showPageCount);\n");
			sb.append("}\n");
		...
		pageAjaxStr = sb.toString();
		return pageAjaxStr;
	}
	
	
	public void setPageAjaxStr(String pageAjaxStr) {
		this.pageAjaxStr = pageAjaxStr;
	}

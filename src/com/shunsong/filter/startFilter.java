...
import com.shunsong.plugin.websocketInstantMsg.ChatServer;
import com.shunsong.plugin.websocketOnline.OnlineChatServer;
import com.shunsong.plugin.websocketSysMsg.WebSocketSysMsgServer;
...
public class startFilter extends BaseController implements Filter{
	 ...
	 * 启动系统消息服务
	 */
	public void startWebsocketSysMsg(){
		WebSocketImpl.DEBUG = false;
		WebSocketSysMsgServer s;
		try {
			String strWEBSOCKET = Tools.readTxtFile(Const.WEBSOCKET);//读取WEBSOCKET配置,获取端口配置
			if(null != strWEBSOCKET && !"".equals(strWEBSOCKET)){
				String strIW[] = strWEBSOCKET.split(",fh,");
				if(strIW.length == 7){
					s = new WebSocketSysMsgServer(Integer.parseInt(strIW[6]));
					s.start();
				}
			}
			//System.out.println( "websocket服务器启动,端口" + s.getPort() );

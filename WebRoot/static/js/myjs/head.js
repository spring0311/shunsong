...
$(function(){
	...
	/*
	setInterval(function(){getMessList();
		var messCount  = $("#messCount").text() ;
		if(Number(messCount) != 0){
			 $("#messUl").click();
		 }
	},5*60*1000);//定时执行提示 5分钟提醒一次
	*/
});
...
function goSaveEstimate(){
	
	var url = locat + '/work/goInputEstimateTimeJqgrid'
...
function goSaveUserWorkTime(){
	var url = locat+"/work/goSaveUserWorkTime.do";
	var diag = new top.Dialog();
	 diag.Drag=true;
	 diag.Title ="录入出勤工时";
	 diag.URL = url;

				...
				<div class="row">
					<div class="col-xs-5">
						...
						<div class="tab-content no-border padding-24">
							...
							<input type="hidden" name="MAKE_BOM_PART_ID" id="MAKE_BOM_PART_ID" value="${parts.MAKE_BOM_PART_ID}"/>							
								<table id="table_head1" class="table table-striped table-bordered table-hover">
									...
									<tr>
										...
										<td>
											<input class="nextInput" type="text"  name="PRODUCTION_FIGURE_NO" id="PRODUCTION_FIGURE_NO" value="${parts.PRODUCTION_FIGURE_NO}" onkeydown="findPartsNoByDrawingNo();" maxlength="" placeholder="这里输入零件号" title="生产日期" style="width:80%;"/>
											<a class="btn btn-light btn-xs" onclick="selectMaterial1();"  title="检索"><i id="nav-search-icon" class="ace-icon fa fa-search bigger-110 nav-search-icon blue"></i></a>
										</td>
									</tr>
									<tr>
							...
							<div id="faq-tab-2" class="tab-pane fade ">
								<table id="table_head2" class="table table-striped table-bordered table-hover">
									...
									<tr>
										<td style="width:150px;text-align: right;padding-top: 13px;">零件号:</td>
										<td>
											<input class="nextInput" type="text"  name="DRAWING_NO_NEW" id="DRAWING_NO_NEW" maxlength="" placeholder="这里输入零件号" title="生产日期" style="width:98%;"/>
										</td>
									</tr>
									<tr>
										<td style="width:150px;text-align: right;padding-top: 13px;">零件名称:</td>
										<td>
										<%--	<select class="chosen-select form-control" name="PARTS_NO" id="PARTS_NO" data-placeholder="请选择配件名称" style="vertical-align:top;width: 98%">
										 	<option value=""></option>
										  	</select> --%>
											<input class="nextInput" type="text"  name="PARTS_NO_NEW" id="PARTS_NO_NEW" value="${pd.PARTS_NO}" maxlength="" placeholder="这里输入零件名称" title="生产日期" style="width:80%;"/>
									...
									<tr>
										<td style="width:150px;text-align: right;padding-top: 13px;">数量:</td>
										<td>
											<input class="nextInput" type="number"  name="NUM_NEW" id="NUM_NEW" value="${pd.NUM_NEW}" maxlength="" placeholder="这里输入配件的数量" title="生产日期" style="width:90%;"/>PCS
										</td>
									</tr>
							...
							<h5 class="lighter">配件使用材料：</h5>
								<table id="table_head" class="table table-striped table-bordered table-hover">
									<tr>
										...
										<td>
											<input class="nextInput" type="text"  name="PART_NAME" id="PART_NAME" value="${parts.PART_NAME}" maxlength="" placeholder="这里输入配件的使用材料" title="生产日期" style="width:80%;"/>
											<input type="hidden"  name="PART_ID" id="PART_ID" value="${parts.PART_ID}" />
									...
									<tr>
										...
										<td>
											<input class="nextInput" type="text" value="${parts.MATERIAL_SPEC }" name="MATERIAL_SPEC" id="MATERIAL_SPEC">（单位：mm）（H×W×L）
									...
									<tr>
										...
										<td >
											<input type="hidden"  name="PART_SPEC" id="PART_SPEC" value="${parts.PART_SPEC}" maxlength="" placeholder="厚度" style="width:30%;"/>
											<input type="text"  class="nextInput" name="SIZE_H" id="SIZE_H" value="${fn:split(parts.PART_SPEC,'*')[0]}" maxlength="" placeholder="厚度" style="width:30%;"/>
											<input type="text"  class="nextInput" name="SIZE_W" id="SIZE_W" value="${fn:split(parts.PART_SPEC,'*')[1]}" maxlength="" placeholder="宽度" style="width:30%;"/>
											<input type="text"  class="nextInput" name="SIZE_L" id="SIZE_L" value="${fn:split(parts.PART_SPEC,'*')[2]}" maxlength="" placeholder="长度" style="width:30%;"/>
										</td>
									</tr>							
								</table>
										...
										<c:when test="${not empty varList}">
											<c:forEach items="${varList}" var="var" varStatus="vs">
												<tr>
													...
													<td class='center' style="display:none;">${var.PROCESS_ID}</td>
													<td class='center'>
													    ${var.PROCESS_NAME}
													</td>
													<td class='center'>
													...
													<td class='center'>
													    <input type="text"  name="ESTIMATE_TIME" id="ESTIMATE_TIME" value="${var.SINGLE_ESTIMATE_TIME }" onkeydown="focusNextInput(this)" maxlength="" placeholder="填工时（分钟）" title="预估工时" style="width:80%;padding: 1px;line-height: 5px;"/>	
													</td>	
													...
													<td style="display: none;"><span name="MAKE_BOM_ID_PROCESS">${var.MAKE_BOM_ID}</span></td>
...
<div id="zhongxin2" class="center" style="display:none"><img src="static/images/jzx.gif" style="width: 50px;" /><br/><h4 class="lighter block green"></h4></div>
	...
	<script type="text/javascript" src="static/js/jquery.tips.js"></script>
		...
		function saveDetail(flag){
			var MAKE_BOM_PART_ID = $("#MAKE_BOM_PART_ID").val();
			...
			var obj = {};
			obj.MAKE_BOM_PART_ID = MAKE_BOM_PART_ID;
			...
			for (var i=1;i<rows.length;i++) {
			    ...
			    var makeBomIdProcess = $(rows[i]).find('span[name="MAKE_BOM_ID_PROCESS"]').text();
				...
				json = json + ',makeBomId:"' + makeBomIdProcess + '"';
				json = json + '}';
			...
			$.ajax({
				type: "POST",
				url: 'makebompart/editPartAndDetail.do',
		    	data:  obj,
				dataType:'json',
				cache: false,
		...
		function addProcess(processID,processName,processCode,processClassId,processClassName){
			...
			$("#processListTable tbody").append(
						...
						'<td style="display: none;"><span name="MAKE_BOM_ID_PROCESS"></span></td>'+	
						' <td class="center">  '+	
						'    <a class="btn btn-mini btn-grey" onclick="upList(this)\;"><i class="icon-pencil"></i>上移</a>  '+
						'    <a class="btn btn-mini btn-grey" onclick="downList(this)\;"><i class="icon-pencil"></i>下移</a> &nbsp;&nbsp;&nbsp; '+
						'    <a class="btn btn-mini btn-pink" onclick="deleteRow(this)\;"><i class="icon-pencil"></i>删除</a>  '+
		...
		function findPartsNoByDrawingNo(){
			var drawingNo = $("#PRODUCTION_FIGURE_NO").val();
			if(event.keyCode==13 &&drawingNo.length > 0){
				$.ajax({
					...
					success: function(data){
						if(data.msg == 'success'){
							if(data.size == 1){
								$("#PRODUCTION_NAME").val(data.PARTS_NO);
								$("#PRODUCTION_ID").val(data.PARTS_CLASS_ID);
								$("#PRODUCTION_NUM").focus();
							}else if(data.size > 1){
								var head = drawingNo.substring(0,1);
								selectMaterial1(head);
							}
						}
					} 
				});

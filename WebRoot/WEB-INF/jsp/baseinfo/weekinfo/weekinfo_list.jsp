...
<body class="no-skin">
			...
			<div class="main-content-inner">
				<div class="page-content">
					<div class="row">
						...
						<!-- 检索  -->
						<form action="weekinfo/list.do" method="post" name="Form" id="Form">
							...
							<c:choose>
								<c:when test="${not empty varList}">
									<c:forEach items="${varList}" var="var" varStatus="vs">
										<tr>
											<td class='center'><div name="WEEK_ID">${var.WEEK_ID}</div></td>
											<td class='center'><div name="WEEK_NUM">${var.WEEK_NAME}</div></td>
											<td class='center'><input type="checkbox" value="Y" name="IS_WORK" <c:if test="${var.IS_WORK=='Y'}" >checked="checked"</c:if> /> </td>
											<td class='center'><input type="number" value="<fmt:formatNumber value='${var.WORK_TIME}' pattern='#0.00' />" name="WORK_TIME" /></td>
										</tr>
									</c:forEach>
								</c:when>
								<c:otherwise>
	...
	<script type="text/javascript">
		...
		function save(){
			...
			$.ajax({
				type: "POST",
				url: '<%=basePath%>'+'weekinfo/editList.do',

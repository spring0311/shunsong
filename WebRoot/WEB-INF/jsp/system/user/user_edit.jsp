								...
								<form action="user/${msg }.do" name="userForm" id="userForm" method="post" >
									...
									<table id="table_report" class="table table-striped table-bordered table-hover">
										...
										<tr>
											...
											<td id="juese1">
												<select class="chosen-select form-control" name="DEPARTMENT_ID" id="DEPARTMENT_ID" data-placeholder="请选择所在部门" style="vertical-align:top;" style="width:98%;" >
													<option value=""></option>
													<c:forEach items="${deptList}" var="deptuser">
														<option value="${deptuser.DEPARTMENT_ID }" <c:if test="${deptuser.DEPARTMENT_ID == pd.DEPARTMENT_ID }">selected</c:if>>${deptuser.NAME }</option>
													</c:forEach>
												</select>												
											</td>
										</tr>
										...
										<tr>
											...
											<td>
												<c:if test="${empty pd.USER_ID }"> 
													<input type="text" class="span10 date-picker" data-date-format="yyyy-mm-dd" readonly="readonly" name="RESUME_DATE_IN" id="RESUME_DATE_IN"  placeholder="选择入职时间" title="入职时间" style="width:98%;"/></td>
												</c:if>
										</tr>
										<tr>
							...
							<div class="col-xs-4">
								<div class="row">
									<div class="row" style="height:400px;text-align: center;">
										<div class="row" style="text-align: center;margin-top: 60%;">
											<img src="" id="staffPicture" style="height:100%;width:60%"  /> 
										</div>
									</div>
									<div class="row" style="text-align: center;">
										<div class="row" style="text-align: center;">
											<a class="btn btn-sm btn-primary" id="btnUpload">上传照片</a>
										</div>
									</div>
								</div>
							</div>
...
<script type="text/javascript">
    ...
    $('#btnUpload').on('click', function(){
         var billId = $("#user_id").val();
         var diag = new top.Dialog();
       	 diag.Drag=true;
       	 diag.Title ="上传照片";
       	 diag.URL = '<%=basePath%>attachment/goUpload.do?BILL_ID='+billId+'&BILL_TYPE=USER_PICTURE';
       	 diag.Width = 550;
       	 diag.Height = 750;
    ...
    function initStaffPicture(){
    	var userId = $("#user_id").val();
    	$.ajax({
			type: "POST",
			url: '<%=basePath%>user/getStaffPictureUrl.do',
	    	data: {BILL_ID:userId},
			dataType:'json',
			cache: false,
			success: function(data){
				 if("success" == data.msg){
					 console.log(data);
					 $("#staffPicture").attr("src",data.url);
				 }
			}
		});
    }

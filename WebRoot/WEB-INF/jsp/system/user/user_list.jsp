					...
					<div class="row">
						...
						<table style="margin-top:5px;">
							<tr>
								...
								<td style="padding-left:2px;"><input class="span10 date-picker" id="lastLoginEnd" name="lastLoginEnd"  value="${pd.lastLoginEnd}" type="text" data-date-format="yyyy-mm-dd" readonly="readonly" style="width:88px;" placeholder="结束日期" title="最近登录结束"/></td>
								...
								<c:when test="${not empty userList}">
									...
									<c:forEach items="${userList}" var="user" varStatus="vs">
										...
										<tr>
											...
											<td class="center"><c:if test="${user.STATUS =='0'}">在职</c:if>
																<c:if test="${user.STATUS =='1'}">离职</c:if></td>
											...
											<td class="center">
												...
												<div class="hidden-sm hidden-xs btn-group">
													...
													<a class="btn btn-xs btn-pink" title="履历" onclick="goUserResumeList('${user.USER_ID}');">
														<i class="ace-icon fa fa-bookmark-o bigger-120" title="履历"></i>
													</a>
													</c:if>
													<c:if test="${QX.del == 1 }">
												...
												<div class="hidden-md hidden-lg">
													<div class="inline pos-rel">
														...
														<ul class="dropdown-menu dropdown-only-icon dropdown-yellow dropdown-menu-right dropdown-caret dropdown-close">
															...
															<li>
																<a style="cursor:pointer;" onclick="goUserResumeList('${user.USER_ID}');" class="tooltip-success" data-rel="tooltip" title="履历">
																	<span class="green">
					...
					<table style="width:100%;">
						<tr>
							<td style="vertical-align:top;">
								...
								<a class="btn btn-mini btn-success" onclick="setExcellentStaff();">设置每月优秀员工</a>
								</c:if>
...
$(top.hangge());
	...
	//修改
	function goUserResumeList(user_id){
		 top.jzts();
		 var diag = new top.Dialog();
		 diag.Drag=true;
		 diag.Title ="履历";
		 diag.URL = '<%=basePath%>userresume/listPerson.do?USER_ID='+user_id;
		 diag.Width = 1024;
		 diag.Height = 768;
		 diag.CancelEvent = function(){ //关闭事件
			diag.close();
	...
	function setExcellentStaff(){
		top.jzts();
		var diag = new top.Dialog();
		diag.Drag=true;
		diag.Title ="设置优秀员工";
		diag.URL = '<%=basePath%>excellentstaff/list.do';
		diag.Width = 1024;
		diag.Height = 768;
		diag.CancelEvent = function(){ //关闭事件
			diag.close();

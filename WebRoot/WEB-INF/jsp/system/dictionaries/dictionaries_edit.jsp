				...
				<div class="row">
					...
					<form action="dictionaries/${msg }.do" name="Form" id="Form" method="post">
						...
						<table id="table_report" class="table table-striped table-bordered table-hover">
							...
							<tr>
								<td style="width:100px;text-align: right;padding-top: 13px;">对应工序:</td>
								<td>
									<select name="PROCESS_ID" id="PROCESS_ID">	
									    <c:if test="${empty pd.PROCESS_ID || pd.PROCESS_ID == ''}">
									    	<option value="" >请选择工序</option>
									    </c:if>								    
										<c:choose>
										<c:when test="${not empty processList}">										    
											<c:forEach items="${processList}" var="processList" varStatus="vs">
											    <option value="${processList.PROCESS_ID}" <c:if test="${pd.PROCESS_ID == processList.PROCESS_ID}">selected</c:if>>${processList.PROCESS_NAME}</option>												
											</c:forEach>
										</c:when>
										<c:otherwise>
											<option value="" >请选择工序</option>
